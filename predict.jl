...
# ╔═╡ cdbe0424-6c43-4481-98d8-d951356d045d
using CSV, DataFrames, MLJ, MLJModels, MLJDecisionTreeInterface, MLJXGBoostInterface, PlutoUI, PlutoTeachingTools

# ╔═╡ ce788890-b2e7-11ef-31a6-0de3aae9bc9e
html"""
...
# ╔═╡ ebb91434-a336-4d15-a8a5-2d1cfb5ce636
md"截止日期: 16周周日结束"

# ╔═╡ c66ae5ed-165a-4cbe-8ad0-e22964fe98ab
TableOfContents(title = "目录")

# ╔═╡ 8afa995c-1a48-45ca-a4c4-35136d22829b
md"""
**作业要求**

- 请利用所学知识，完成天池大数据平台【教学赛】金融数据分析赛题1：银行客户认购产品预测。 
- 撰写竞赛报告，涵盖：摘要， 赛题介绍， 数据分析，模型及效果、总结等几部分。报告中应该尝试不少于3个模型的结果。
- 提交报告的jl文档和对应的pdf文档，命名为 ***大作业.jl, ***大作业.pdf（使用Python的同学提交类似的notebook源代码和导出的PDF文档）
"""

# ╔═╡ b0d99e87-4197-4522-96af-9f6ec1313f33
student = (
    name="郭情峰",
    id=20221003375, # 学号
	tianchiname = "gdufs_echo", # 请填写自己的用户名
	score = 0.9633 # 请填写竞赛系统中的最好的得分
)

# ╔═╡ 5494c795-2c4d-427e-8225-869a993172f8
if student.name == "张三"
    still_missing(md"""请将`"张三"` 替换为你自己的名字， 请不要带空格。""")
elseif student.id == 12345678910
    still_missing(md"请输入你真实的学号。")
elseif !isa(student.id, Number)
    still_missing(md"请用数字输入学号， 不要用字符串。")
elseif student.tianchiname == "天池平台用户名" 
	still_missing(md"""请输入你自己的天池平台用户名。""")
elseif student.score == 0
	still_missing(md"""请输入你自己的竞赛得分。""")
else
    correct()
end

# ╔═╡ a8a5bf53-5af5-4023-875b-5ba30efa5e28
md"""
## 摘要
"""

# ╔═╡ c47f39e4-ee45-4b6b-a520-9eed86fa4ab0
md"""
1. **管道模型设计**
   - 使用 `|>` 管道符构建模型流程
   - 一行代码完成特征选择、编码和模型训练
   ```julia
   model = delfs |> cat_selector |> text2categorical |> 
           ContinuousEncoder |> classifier
   ```
...
3. **高效评估**
   ...
   - 自动计算 AUC
   ```julia
   eres5 = evaluate(model5, trains, y, 
...
# ╔═╡ def3e89d-539c-4469-8dd1-63a22f934bab
md"""
## 赛题背景
"""

# ╔═╡ cb388ce4-94b0-4d6e-bd0d-8e5cafeaeaf6
md"""
本赛题来源于银行的营销数据集，背景是银行通过电话营销方式向客户推广定期存款业务。数据集包含了客户的个人信息（如年龄、职业、婚姻状况等）、银行联系相关信息（如通话时长、联系次数等）以及社会经济背景指标。

任务目标是构建一个预测模型，通过分析客户特征来预测客户是否会订购银行的定期存款（二分类问题：是/否）。这个预测模型可以帮助银行更有效地识别潜在客户，提高营销效率，降低营销成本。
"""

# ╔═╡ 210c832f-9916-42d2-b57d-79c4e31ffdfd
md"""
## 1. 加载必要的包
"""

# ╔═╡ d9eb781d-ead0-40b6-aeca-683c48f4b2cc
md"""
## 2. 数据准备
"""

# ╔═╡ ab1320e4-2b7b-4460-aa1b-bac8c75446d1
trains = CSV.read("train.csv", DataFrame)

# ╔═╡ d68b7bee-24de-4170-9253-24786e978900
tests = CSV.read("test.csv", DataFrame)

# ╔═╡ fdb78d89-9c0d-42e4-9da2-f6958f2f7eac
y = categorical(trains.subscribe)

# ╔═╡ a5b6f97c-86cc-4e23-8223-0725db35a141
md"""
## 3. 模型准备
"""

# ╔═╡ b678ba57-9cdf-4455-a7e7-90bff96807dd
md"""
### 3.1 定义分类特征和数值特征
"""

# ╔═╡ 0e57edf5-1c1a-4bb5-8b93-c116e3f75962
begin
categorical_features = [:job, :marital, :contact, :month, :day_of_week, :poutcome, :housing, :default]
...
# ╔═╡ 70a82f8d-335c-46b0-ac91-fd4cc665e9ee
md"""
### 3.2 特征选择组件
"""

# ╔═╡ 9164ec0b-aeec-451c-9fa5-bc4bc13775f9
begin 
	delfs = FeatureSelector(features = [:subscribe, :id], ignore = true)
	cat_selector = FeatureSelector(features=categorical_features)
	num_selector = FeatureSelector(features=numeric_features)
end

# ╔═╡ ce02c8ef-ac6b-48ec-ab17-a73ecc6b9966
md"""
### 3.3 加载编码器
"""

# ╔═╡ ce0a7ea5-fb3c-4c9a-96d8-aec9c8532a83
begin
	OneHotEncoder = @load OneHotEncoder verbosity=0
	ContinuousEncoder = @load ContinuousEncoder verbosity=0
end

# ╔═╡ c0566010-be9a-4b7d-9d3c-1ac10ffb8ff6
md"""
### 3.4 定义文本到类别的转换函数
"""

# ╔═╡ e4d15284-32e9-458f-9ca1-eccf6f7d8360
text2categorical(x) = coerce(x, Textual => Multiclass)

# ╔═╡ 3ccba05b-3fdb-4bbc-b89e-200f11d8e5a8
md"""
## 4. 构建模型
"""

# ╔═╡ 5ec064be-50a5-4a3b-86d9-927ae892de95
md"""
### 4.1 加载分类器
"""

# ╔═╡ c9525802-cead-4265-a7bb-0812ea875453
begin
	Tree = @load DecisionTreeClassifier pkg=DecisionTree verbosity=0
	Forest = @load RandomForestClassifier pkg=DecisionTree verbosity=0
	XGBoost = @load XGBoostClassifier verbosity=0
	AdaBoostStump = @load AdaBoostStumpClassifier pkg=DecisionTree verbosity=0
end

# ╔═╡ 0026506c-1982-4ac6-9f23-26d5cc8d2766
md"""
### 4.2 构建分类器实例
"""

# ╔═╡ a7b32753-6ef7-46eb-9ab4-1f26dc6c3723
begin 
	tree1 = Tree()
	tree2 = Tree(max_depth=5)
	forest = Forest(n_trees=100)
	xgb = XGBoostClassifier(num_round=100, max_depth=6)
	adaboost = AdaBoostStump(n_iter=200)
end

# ╔═╡ 05d5e88f-83ae-47cd-8394-0ebcca8e177d
md"""
### 4.3 构建管道模型
"""

# ╔═╡ b6c53dd5-36a3-4273-92de-9f54db7a8f5a
begin
	model1 = delfs |> cat_selector |> text2categorical |> ContinuousEncoder |> tree1
	model2 = delfs |> cat_selector |> text2categorical |> ContinuousEncoder |> tree2
	model3 = delfs |> cat_selector |> text2categorical |> ContinuousEncoder |> forest
	model4 = delfs |> cat_selector |> text2categorical |> ContinuousEncoder |> xgb
	model5 = delfs |> cat_selector |> text2categorical |> ContinuousEncoder |> adaboost
end

# ╔═╡ d41a008f-4095-4ad6-aadd-26c960bae398
md"""
## 5. 模型训练和评估
"""

# ╔═╡ f7dc5623-b703-4606-a84c-537e87100efb
md"""
### 5.1 评估模型
"""

# ╔═╡ f1c8c6e8-159b-4047-8f11-0c92cbaeb0be
begin
	...
	eres3 = evaluate(model3, trains, y, resampling=CV(nfolds=5), measure=[auc])
	eres4 = evaluate(model4, trains, y, resampling=CV(nfolds=5), measure=[auc])
	eres5 = evaluate(model5, trains, y, resampling=CV(nfolds=5), measure=[auc])
end

# ╔═╡ b161d3f6-5491-4912-a533-de75a38a5ea0
md"""
### 5.2 找出AUC最高的模型
"""

# ╔═╡ ac4ffadb-10ae-4553-9ff3-593a6221e616
begin 
	auc_scores = [eres1.measurement[1], eres2.measurement[1], eres3.measurement[1], eres4.measurement[1], eres5.measurement[1]]
	models = [model1, model2, model3, model4, model5]
	best_model_index = argmax(auc_scores)
	best_model = models[best_model_index]
	println("Best model is model ",best_model_index,"\nAUC Score: ", round(maximum(auc_scores), digits=4))
end

# ╔═╡ db296eba-edc8-471f-864f-eae5d6eb6e80
md"""
### 5.3 使用最佳模型进行训练和预测
"""

# ╔═╡ 33db0526-8f83-4ad4-9cea-c0996273085c
begin 
	mach = machine(best_model, trains, y)
	fit!(mach)
end

# ╔═╡ 1b40c3f8-55ea-4eb8-b324-9adc1b817bf7
md"""
### 5.4 对测试集进行预测
"""

# ╔═╡ cb5f4512-3405-4019-b7d3-d6505f9b2865
begin 
	predictions = predict(mach, tests)
	pred_probs = predict_mode(mach, tests)
end

# ╔═╡ bc1a24c8-0f10-4262-a042-906fd52aa9ad
md"""
### 5.4 结果保存
"""

# ╔═╡ a98504dd-4e18-4487-8238-35562c4ff7f0
begin 
	submission = DataFrame(
    	id = tests.id,
    	subscribe = pred_probs
	)
	CSV.write("submission.csv", submission)
end

# ╔═╡ f221c358-15ad-4ab9-ab7e-dc149655d0cc
md"""
## 总结
"""

# ╔═╡ 0369dbc8-5a0b-4487-b13c-deefc25bf03d
md"""
本方案针对银行营销预测这一二分类问题，采用了模块化的机器学习流程。通过管道设计简化了数据处理流程，比较了多个经典分类器后选用 AdaBoostStump 作为最终模型。方案的优势在于：
1. 代码简洁高效，易于维护
2. 模型选择科学，性能稳定
3. 评估方法严谨，使用分层交叉验证
4. 特征工程合理，自动化程度高

该方案不仅解决了预测问题，也为银行提供了一个可靠的客户筛选工具，有助于提高营销效率，降低营销成本。
"""
...
manifest_format = "2.0"
project_hash = "867d812b7821c69d1cc5431aacc93ddd6af5a83f"
...
git-tree-sha1 = "385031c897e822755e7d194b0f9745fb20129972"
...
deps = ["LinearAlgebra", "SparseArrays", "SuiteSparse"]
git-tree-sha1 = "6bc0c4cca30222586bde1ec5a0d9a3c6aaa51833"
